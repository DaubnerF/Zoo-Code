...
describe("Cline", () => {
	...
	describe("abortTask", () => {
		...
		describe("cancelCurrentRequest", () => {
			...
			describe("abortSignal", () => {
				...
				it("sends policy-filtered tools to Gemini requests", async () => {
					...
					// Gemini receives the same filtered declaration list as every other
					// provider: a disabled tool is absent from the sent declarations, so
					// the model can never call it.
			...
			it("forwards the caller's options to the context-window retry recursion", async () => {
				...
				const retryOptions = attemptApiRequestSpy.mock.calls[1]?.[1]
				expect(retryOptions).toMatchObject({
					skipProviderRateLimit: options.skipProviderRateLimit,
					hasRequestState: true,
				})
				expect(retryOptions?.requestModelInfo).toBe(options.requestModelInfo)
				expect(retryOptions?.requestState).toBeDefined()
			})
			...
			it("paces the auto-approval retry with the captured request delay when provider state changes", async () => {
				const task = await createRetryForwardingTask({ autoApprovalEnabled: true, requestDelaySeconds: 3 })
				// The entry capture is the next getState() read; every read after it
				// sees a changed delay that the retry pacing must ignore.
				vi.spyOn(mockProvider, "getState")
					.mockResolvedValueOnce(providerStateWith({ autoApprovalEnabled: true, requestDelaySeconds: 3 }))
					.mockResolvedValue(providerStateWith({ autoApprovalEnabled: true, requestDelaySeconds: 60 }))
				// An ask landing here means the auto-approval branch was not taken,
				// so the rejection names the wrong-site failure explicitly.
				vi.spyOn(task, "ask").mockRejectedValue(new Error("auto-approval retry must not prompt the user"))
				vi.spyOn(task.api, "createMessage")
				...
				const retryDelaySays = saySpy.mock.calls.filter(
					([type, text]) =>
						type === "api_req_retry_delayed" && typeof text === "string" && text.includes("<retry_timer>"),
				)
				expect(retryDelaySays).toHaveLength(3)
				expect(retryDelaySays.map(([, text]) => text)).toEqual([
					expect.stringContaining("<retry_timer>3</retry_timer>"),
					expect.stringContaining("<retry_timer>2</retry_timer>"),
			...
			it("reuses the initial provider-state snapshot when provider state changes after the first failed attempt", async () => {
				const task = await createRetryForwardingTask({ autoApprovalEnabled: true })
				vi.spyOn(task, "ask").mockRejectedValue(new Error("auto-approval retry must not prompt the user"))
				// The first read is the attempt's own capture; every later read
				// returns divergent disabledTools, so a re-read on the retry hop
				// would move the prompt and tool-policy inputs mid-request.
				const initialState = providerStateWith({
					autoApprovalEnabled: true,
					requestDelaySeconds: 0,
					disabledTools: ["execute_command"],
				})
				const changedState = providerStateWith({
					autoApprovalEnabled: true,
					requestDelaySeconds: 0,
					disabledTools: ["read_file"],
				})
				const getStateMock = vi.spyOn(mockProvider, "getState")
				getStateMock.mockReset()
				getStateMock.mockResolvedValueOnce(initialState).mockResolvedValue(changedState)
				...
				expect(attemptApiRequestSpy).toHaveBeenCalledTimes(2)
				expect(getSystemPromptMock).toHaveBeenCalledTimes(2)
				// Both hops build the prompt from the first captured snapshot; a
				// retry-time re-read would have observed changedState instead.
				expect(getSystemPromptMock.mock.calls[0]?.[0]).toBe(initialState)
				expect(getSystemPromptMock.mock.calls[1]?.[0]).toBe(initialState)

				const retryOptions = attemptApiRequestSpy.mock.calls[1]?.[1]
				expect(retryOptions?.hasRequestState).toBe(true)
				expect(retryOptions?.requestState).toBe(initialState)
			})

			it("does not re-read provider state on the retry hop when the captured snapshot is undefined", async () => {
				const task = await createRetryForwardingTask()
				vi.spyOn(task, "ask").mockResolvedValue({ response: "yesButtonClicked" } satisfies TaskAskResult)
				const changedState = providerStateWith({ requestDelaySeconds: 0, disabledTools: ["read_file"] })
				const getStateMock = vi.spyOn(mockProvider, "getState")
				getStateMock.mockReset()
				getStateMock.mockResolvedValue(changedState)
				...
				// The caller threaded an explicitly captured empty snapshot: the
				// presence marker must outrank the falsy value, or the retry hop
				// would re-read and silently switch to changedState.
				const iterator = task.attemptApiRequest(0, {
					skipProviderRateLimit: true,
				...
				expect(getSystemPromptMock.mock.calls[0]?.[0]).toBeUndefined()
				expect(getSystemPromptMock.mock.calls[1]?.[0]).toBeUndefined()
			})
	...
	describe("safeEnsureModelFetched", () => {
		...
		it("does not start metadata loading when the task aborts during the rate-limit wait", async () => {
			const rateLimitClock = createRateLimitClock()
			rateLimitClock.recordRequest()
			const task = new Task({
				provider: mockProvider,
			...
			// A user press landing mid-countdown flips the abort flag while the
			// rate-limit wait is still running.
			const saySpy = vi.spyOn(task, "say").mockImplementation(async (type) => {
			...
			await expect(iterator.next()).rejects.toThrow("aborted during request construction")

			expect(saySpy.mock.calls.some(([type]) => type === "api_req_rate_limit_wait")).toBe(true)
			expect(ensureModelFetched).not.toHaveBeenCalled()
			expect(createMessageSpy).not.toHaveBeenCalled()
		})
