...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
						...
						if (this.abort) {
							...
							if (stateForBackoff?.autoApprovalEnabled) {
								await this.backoffAndAnnounce(
									...
									error,
									stateForBackoff?.requestDelaySeconds,
								)
	...
	private async safeEnsureModelFetched(): Promise<ModelInfo> {
		// A task aborted or abandoned before this call (typically cancelled during
		// the preceding rate-limit wait) must not create a controller or start a
		// metadata fetch; return the current model info instead.
		if (this.abort || this.abandoned) {
	...
	public async *attemptApiRequest(
		...
		options: {
			...
			requestModelInfo?: ModelInfo
			requestState?: Awaited<ReturnType<ClineProvider["getState"]>>
			hasRequestState?: boolean
		} = {},
	): ApiStream {
		// Reuse the caller's provider-state snapshot when threaded. `hasRequestState`
		// marks presence explicitly so a captured `undefined` (provider already gone)
		// does not trigger a second read on recursive retries.
		const state = options.hasRequestState ? options.requestState : await this.providerRef.deref()?.getState()
		...
		// Retry recursions must reuse these snapshots instead of re-deriving them: a
		// metadata fetch or settings change landing between attempts would otherwise
		// move model-specific tool policy, `preserveReasoning`, or the prompt/tool
		// inputs mid-request. When either snapshot was captured here its options
		// object is replaced once; when the caller threaded both, the object is
		// forwarded unchanged — same reference, and never mutated.
		const retryOptions =
			options.requestModelInfo === undefined || !options.hasRequestState
				? { ...options, requestModelInfo, requestState: state, hasRequestState: true }
				: options
		const systemPrompt = await this.getSystemPrompt(state, requestModelInfo)
		...
		if (contextTokens) {
			...
			let contextMgmtTools: import("openai").default.Chat.ChatCompletionTool[] = []
			{
				const provider = this.providerRef.deref()
				if (provider) {
					contextMgmtTools = await buildNativeToolsArray({
						provider,
						cwd: this.cwd,
						mode,
						customModes: state?.customModes,
		...
		// Build complete tools array: native tools + dynamic MCP tools, filtered by
		// the effective mode/tool policy. Every provider, Gemini included, receives
		// only the filtered declarations; the execution-time validator enforces the
		// same policy.
		...
		try {
			...
			if (autoApprovalEnabled) {
				...
				await this.backoffAndAnnounce(retryAttempt, error, requestDelaySeconds)
	...
	private async backoffAndAnnounce(
		...
		try {
			// Paced by the caller's request-time snapshot; a state re-read here
			// would let a settings change move the retry delay mid-request.
