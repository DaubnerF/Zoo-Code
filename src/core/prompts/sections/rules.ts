...
export function getRulesSection(cwd: string, settings?: SystemPromptSettings, policy?: EffectiveToolPolicy): string {
	const has = (tool: string): boolean => policy?.tools.has(tool) ?? true
	const hasExecuteCommand = has("execute_command")
	const hasAskFollowupQuestion = has("ask_followup_question")
	...
	rules.push(
		`Do not ask for more information than necessary. Use the tools provided to accomplish the user's request efficiently and effectively. When you've completed your task, ${
			hasAttemptCompletion
				? "you must use the attempt_completion tool to present the result"
				: "present the result"
		} to the user. The user may provide feedback, which you can use to make improvements and try again.`,
	)
	...
	rules.push(
		`NEVER end ${hasAttemptCompletion ? "attempt_completion result" : "your result"} with a question or request to engage in further conversation! Formulate the end of your result in a way that is final and does not require further input from the user.`,
	)
	rules.push(
